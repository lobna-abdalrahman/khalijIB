    ...
    public class TransferAuthorizeController : Controller
    {
        ...
        public ActionResult Details(int id)
        {
            ...
            foreach (var item in accas)
            {
                ...
                var FromAccountNumber = Convert.ToInt64(item.FromAccountNumber.Substring(13));
                ...
                var ToAccountNumber = Convert.ToInt64(item.ToAccountNumber.Substring(13));
            ...
            if (tryno < 3)
            {
                ...
                if (vCode.Equals(OTPPassword))
                {
                    ...
                    try
                    {
                        ...
                        string res = myCheque.DoAccountTransfer(model.FromAccountNumber, model.ToAccountNumber, model.TransferAmount,model.FromAccountNumber.Substring(13,3), Session["userName"].ToString(), Session["userName"].ToString());
                        JObject jobj = new JObject();
                        jobj = JObject.Parse(res);
                        dynamic result = jobj;
