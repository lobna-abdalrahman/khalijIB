        ...
        public ActionResult Statement(AccountStatementViewModel ASVM)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    {   Session["fromDate"] = DateTime.ParseExact(fromDate.ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None).ToString("dd-MMM-yyyy");
                      Session["toDate"] = DateTime.ParseExact(toDate.ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None).ToString("dd-MMM-yyyy");
                }
                else
            {
                message = "All Fields are required ";
                ...
                try
                    {
                        string statement =obj.GetStatement(ASVM.AccountNo, noOfTrans, fromDate, toDate, tranType);
                        ...
                        if (index > 0)
                        {
                            for (int i = 1; i <= index; i++)
                            {
                                ...
                                try
                                {
                                    ...
                                    accas.Add(new AccountStatementViewModel
                                    {
                                        ...
                                        AccountCurrency = data.GetCurrencyName(ASVM.AccountNo.Substring(10, 3).ToString())
                            ...
                            //CreateReport(ASVM.fromDate, ASVM.toDate);
                            //Response.Redirect("~/Shared/ShowReports.aspx");
                        }
                        else
                        {
